...
import de.dentrassi.camel.iec60870.server.ServerOptions;
...
public class ServerInstance {
	...
	private final class DataModelImpl extends ChangeDataModel {
		...
		protected ChangeModel createChangeModel() {
			if (ServerInstance.this.options.getBufferingPeriod() != null
				...
				logger.info("Creating buffering change model: {} ms", ServerInstance.this.options.getBufferingPeriod());
		...
		protected BackgroundModel createBackgroundModel() {
			if (ServerInstance.this.options.getBackgroundScanPeriod() > 0) {
				logger.info("Creating background scan model: {} ms",
						ServerInstance.this.options.getBackgroundScanPeriod());
		...
		@Override
		public void notifyDataChange(final ASDUAddress asduAddress,
				final InformationObjectAddress informationObjectAddress, final Value<?> value, final boolean notify) {
	...
	public interface ServerObjectListener {
		public CompletionStage<Void> execute(Request<?> request);
	}
	...
	public void start() {
		...
		this.server = new Server(this.address, this.options.getProtocolOptions(),
				asList(this.dataModule, new DiscardAckModule()));
	}
